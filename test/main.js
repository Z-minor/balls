...
function loop() {
  ...
  if((hardMode==0&&balls.length<=42)||hardMode==1||1){
    ...
    if(補血道具重生計時<0){
      ...
      if(計時器%80<30){
        愛心脈動比例 = 1.5-Math.abs((計時器%80/30)-0.5);
      }
      else if(計時器%80<50){
        愛心脈動比例 = 1.3-Math.abs(((計時器%80-30)/20*0.6)-0.3);
      }
      else{
        愛心脈動比例 = 1;
      }
